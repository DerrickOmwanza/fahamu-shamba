...
// Get current prices
router.get('/market/prices', ensureInitialized, (req, res) => {
  try {
    const { crop, market } = req.query;
    const result = marketService.getCurrentPrices({ crop, market });
    res.json(result);
  } catch (error) {
...
// Get price history
router.get('/market/prices/history', ensureInitialized, (req, res) => {
  try {
    ...
    const result = marketService.getPriceHistory(crop, market, parseInt(days) || 30);
    res.json(result);
  } catch (error) {
...
router.get('/market/prices/market/:market', ensureInitialized, (req, res) => {
  try {
    const { market } = req.params;
    const result = marketService.getPricesByMarket(market);
    res.json(result);
  } catch (error) {
...
router.get('/market/prices/compare', ensureInitialized, (req, res) => {
  try {
    ...
    const result = marketService.getPriceComparison(crop);
    res.json(result);
  } catch (error) {
