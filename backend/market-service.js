...
function seedMarketData() {
  const existingMarkets = getDb().prepare('SELECT COUNT(*) as count FROM market_centers').get();
  ...
  if (existingMarkets.count === 0) {
    ...
    const insertMarket = getDb().prepare(`
      INSERT INTO market_centers (name, location, county, sub_county, operating_days, main_crops)
      VALUES (?, ?, ?, ?, ?, ?)
    `);
    
    markets.forEach(m => {
      insertMarket.run(m.name, m.location, m.county, m.sub_county, m.operating_days, m.main_crops);
    });
    ...
    const prices = [
      ...
      { crop: 'Maize', market: 'Bondo Market', price: 60, unit: 'kg' },
      { crop: 'Beans', market: 'Bondo Market', price: 80, unit: 'kg' },
      ...
      { crop: 'Maize', market: 'Ugunja Market', price: 62, unit: 'kg' },
      // Gem
      { crop: 'Maize', market: 'Gem Market', price: 68, unit: 'kg' },
      { crop: 'Kales', market: 'Gem Market', price: 50, unit: 'kg' },
      { crop: 'Beans', market: 'Gem Market', price: 82, unit: 'kg' },
      // Rarieda
      { crop: 'Maize', market: 'Rarieda Market', price: 66, unit: 'kg' },
      { crop: 'Beans', market: 'Rarieda Market', price: 84, unit: 'kg' },
      { crop: 'Sorghum', market: 'Rarieda Market', price: 92, unit: 'kg' },
      // Ugenya
    ...
    const insertPrice = getDb().prepare(`
      INSERT INTO market_prices (crop, market, price, unit)
      VALUES (?, ?, ?, ?)
    `);
    
    prices.forEach(p => {
      insertPrice.run(p.crop, p.market, p.price, p.unit);
    });
    
    console.log('✅ Market seed data added');
  }
}
